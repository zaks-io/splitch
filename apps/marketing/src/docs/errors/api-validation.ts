...
export const validationErrorDocs = {
  VALIDATION_ERROR: {
    remediation: "Correct the fields named in details.issues and resend the request",
  ...
  ALLOCATION_INVALID: {
    remediation: "Adjust the Variant allocation percentages so they total exactly 100",
  ...
  INVALID_SORT: {
    remediation: "Sort on a field listed in details.allowedFields",
  ...
  EXPOSURE_TICKET_INVALID: {
    remediation: "Discard the ticket and call evaluate-all again for a fresh resolution",
    cause:
  ...
  EXPOSURE_TICKET_EXPIRED: {
    remediation: "Call evaluate-all again; the ticket passed its 24-hour redemption window",
  ...
  UNSUPPORTED_OBJECT_KEY: {
    remediation: "Rename the key named in details.key so it is not __proto__",
    cause:
