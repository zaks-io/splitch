...
export const systemErrorDocs = {
  MULTIPLE_VARIANT_CONFLICT: {
    remediation:
      "Correct the Targeting Key and start a new Run; results covering this Entity are not safe to decide on",
    cause:
  ...
  ATTENTION_FANOUT_LIMIT_EXCEEDED: {
    remediation:
      "Read attention per Environment instead; retrying the App-wide rollup never clears this",
    cause:
  ...
  RATE_LIMITED: {
    remediation: "Back off for details.retryAfterMs, then retry",
  ...
  SERVICE_UNAVAILABLE: {
    remediation:
      "Retry after details.retryAfterMs; the edge is unavailable and your Flag is not misconfigured",
    cause:
  ...
  PRIVACY_JOB_FAILED: {
    remediation:
      "Re-run the job for details.requestId; details.failedStores names the stores it could not complete",
    cause:
  ...
  INTERNAL_SERVER_ERROR: {
    remediation: "Retry once, and report the code with the request context if it persists",
