...
// Returns the remediation as REQUIRED so the wire gate in `index.ts` sees a
// `string`, not the optional `string | undefined` the shared `ErrorDoc` allows.
function notFound(
  resource: string,
  remediation: string,
  hint: string,
): ErrorDoc & { remediation: string } {
  return {
    remediation,
...
export const lookupErrorDocs = {
  SLUG_CONFLICT: {
    remediation: "Resend the create with a different Organization slug",
    cause:
  ...
  EXPERIMENT_KEY_CONFLICT: {
    remediation: "Resend the create with a different Experiment key",
    cause:
  ...
  SENTRY_INSTALLATION_CONFLICT: {
    remediation:
      "Revoke the installation named in details.activeInstallationId before installing a new one",
    cause:
  ...
  EXPERIMENT_NOT_FOUND: notFound(
    "Experiment",
    "Confirm the Experiment id and the Environment it lives in",
  ...
  RUN_NOT_FOUND: notFound(
    "Run",
    "Confirm the Run id and the Experiment that owns it",
    "Confirm the Run id and its Experiment. Runs are addressed under the Experiment that owns them.",
  ),
  FLAG_NOT_FOUND: notFound(
    "Flag",
    "Confirm the Flag id or key, the App, and that the Flag is promoted into this Environment",
  ...
  VARIANT_NOT_FOUND: notFound(
    "Variant",
    "Confirm the Variant name against the Flag's catalog",
  ...
  METRIC_NOT_FOUND: notFound(
    "Metric",
    "Confirm the Metric id and the App it belongs to",
    "Confirm the Metric id and the App it belongs to.",
  ),
  ...
  USER_NOT_FOUND: notFound(
    "user",
    "Confirm the user id within this Organization's membership",
    "Confirm the user id within this Organization's membership.",
  ),
  CREDENTIAL_NOT_FOUND: notFound(
    "credential",
    "Confirm the key id; an API Key secret is shown once and cannot be looked up later",
  ...
  CONVEX_INSTALLATION_NOT_FOUND: notFound(
    "Convex installation",
    "Confirm the installation id and that the API Key addresses the App and Environment that own it",
  ...
  SENTRY_INSTALLATION_NOT_FOUND: notFound(
    "Sentry installation",
    "Confirm the installation id and that the operator token addresses the App and Environment that own it",
  ...
  PRIVACY_JOB_NOT_FOUND: notFound(
    "privacy job",
    "Confirm the job id; completed jobs are unreachable after their retention window",
  ...
  APPROVAL_REQUEST_NOT_FOUND: notFound(
    "Approval Request",
    "Confirm the request id and the Environment whose Policy created it",
