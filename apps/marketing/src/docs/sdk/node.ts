...
app.get("/checkout", async (req, res) => {
  ...
  res.send(enabled ? renderNewCheckout() : renderCurrentCheckout());
    ...
    {
      kind: "prose",
      text: 'This call never throws and never retries. On any failure it returns your `defaultValue`, logs through `logger.error`, and reports `reason: "ERROR"` in `evaluateDetails`. Your handler keeps serving; the loud log is what stops that from becoming a silent outage. See [Failure behavior](/docs/sdk/failures).',
    },
