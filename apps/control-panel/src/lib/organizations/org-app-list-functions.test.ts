...
vi.mock("#lib/shared/control-plane-apps", async (importOriginal) => ({
  ...(await importOriginal<typeof import("#lib/shared/control-plane-apps")>()),
  createControlPanelFlagsClient: (...args: unknown[]) => createControlPanelFlagsClientMock(...args),
}));

const { loadOrgAppListForRequest } = await import("#lib/organizations/org-app-list-functions");
