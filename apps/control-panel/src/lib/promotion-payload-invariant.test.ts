...
import { promotionSelect, selectedRows } from "./promotion-selection";
import { type FlagPromotionScope, promotionRequest } from "./use-flag-promotion";
...
vi.mock("./control-plane-flag-mutations", () => ({
...
const DECLARATION = "lib/control-plane-flag-mutations.ts";
...
describe("the diff shown is the diff submitted", () => {
  ...
  it("leaves promotionRequest the only constructor of a promote select", () => {
    ...
    expect(constructors).toEqual(["lib/promotion-selection.ts", "lib/use-flag-promotion.ts"]);
