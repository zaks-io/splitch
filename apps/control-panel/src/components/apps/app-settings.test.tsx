...
vi.mock("#lib/settings/control-plane-settings-functions", () => ({
...
vi.mock("#lib/environments/control-plane-exposure-status-functions", () => ({
  loadEnvironmentExposureStatus: vi.fn(),
}));

const { AppSettings } = await import("#components/apps/app-settings");
const { AppMemberGrantForm, labelAppAccessCandidates } = await import(
  "#components/apps/app-member-grant-form"
);
