...
describe("CLI-to-SDK dark-launch integration", () => {
  ...
  it("proves disabled → targeted cohort → exposure-safe evaluate → kill-switch-off", async () => {
    ...
    expect(
      await runCli(
        ["flags", "create", "--json", "--app", harness.appId, "--key", FLAG_KEY, "--variants", "on,off"],
        cli,
      ),
    ).toBe(EXIT_OK);
    ...
    const onVariantId = await variantId(harness, flag.id, "on");
    expect(
      await runCli(
        [
            ...
            targetingRules: [
              {
                ...
                priority: 0,
                conditions: [
                  { attribute: COHORT_ATTRIBUTE, operator: "eq", value: COHORT_VALUE },
                ],
    ...
    const commitsBeforeEvaluate = harness.evaluationCommitSink.writes.length;
    const exposuresBeforeEvaluate = harness.exposureSink.writes.length;
    expect(commitsBeforeEvaluate).toBe(0);
    expect(exposuresBeforeEvaluate).toBe(0);
    ...
    expect(harness.evaluationCommitSink.writes.every((write) => write.usage.flagKey === FLAG_KEY)).toBe(
      true,
    );
    ...
    await proveNegativeAuth(harness, clientKey.keyMaterial, flag.id);
  }, 60_000);
});
...
async function proveNegativeAuth(
  ...
  const wrongClient = packedSdk.createSplitchClient({
    clientKey: wrongKeyMaterial ?? "",
    endpoint: quickstartOrigins.evaluationBaseUrl,
    fetch: harness.routingFetch,
  });
  const wrongDetails = await wrongClient.verify(FLAG_KEY, {
    targetingKey: TARGETED_KEY,
    attributes: { [COHORT_ATTRIBUTE]: COHORT_VALUE },
  });
  expect(wrongDetails.reason).toBe("ERROR");

  const rotated = await controlPlanePost<{
    newKey: { keyMaterial: string };
    revokedKeyId: string;
  }>(harness, `/apps/${harness.appId}/envs/${harness.devEnvironmentId}/client-key/revoke`, {});
  expect(rotated.newKey.keyMaterial).not.toBe(priorKeyMaterial);
  const revokedClient = packedSdk.createSplitchClient({
    clientKey: priorKeyMaterial,
    endpoint: quickstartOrigins.evaluationBaseUrl,
    fetch: harness.routingFetch,
  });
  const revokedDetails = await revokedClient.verify(FLAG_KEY, {
    targetingKey: TARGETED_KEY,
    attributes: { [COHORT_ATTRIBUTE]: COHORT_VALUE },
  });
