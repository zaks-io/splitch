...
export function operationBehaviorNotes(command: CliCommandDefinition): string[] {
  if (command.operationId === "experiments_start") {
    return [
      "Start freezes the Experiment draft Targeting Rules into the new Run (returned as frozenTargetingRules).",
  ...
  if (command.operationId === "experiments_update") {
    return [
      "While a Run is live, assignment fields (including targetingRules) require stageForNextRun: true and write only the next-Run draft.",
  ...
  if (command.operationId === "flag_targeting_rules_replace") {
    return [
      ...
      "A live Run evaluates its own frozen targetingRules snapshot, not this Flag Configuration list.",
    ];
  }
  return [];
}
