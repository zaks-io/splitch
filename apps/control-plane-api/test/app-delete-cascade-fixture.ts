...
/** SPL-326 cascade test fixture: two distinct tenants + App delete helpers. */
...
/** Distinct second tenant — isolation proofs must not share ids with ORG (SPL-11). */
export const OTHER = {
...
export async function seedCascadeTenants(): Promise<void> {
  ...
  await repo.privacy.entityDeletions.insert(appScope(OTHER.appId), {
    appId: OTHER.appId,
    idType: "user",
    targetingKeyHash: "hash_cascade_other_tenant",
  ...
  await bindings.d1
    ...
    .bind(
      "prv_cascade_other_tenant",
      OTHER.orgId,
      OTHER.appId,
      "subject_cascade_other_tenant",
      OTHER_OWNER,
      NOW_ISO,
      NOW_ISO,
      NOW_ISO,
