...
export interface OverviewExperimentReading {
  id: string;
  name: string;
  runId: string;
  horizon: string;
 ...
 * a result whose assignment is broken.
 */
export function classifyOverviewExperiments(
  ...
  return { needingDecision, failing };
}
 ...
 * reverse.
 */
function decisionReasons(reading: OverviewExperimentReading): OverviewDecisionReason[] {
 ...
 */
function horizonReached(reading: OverviewExperimentReading): boolean {
