 ...
 *
 * A Miniflare local D1 carries only the roots the mounted handlers read
 * (organizations, apps); the full migration set is gated by @splitch/db's own
 * suite, so this test stays self-contained. A Miniflare local KV backs the
 * session-validation hot read. No real WorkOS, no network.
 */
