...
describe("holdover write App deletion saga boundaries", () => {
  it("ignores a delayed cancel from an older generation after a newer begin", async () => {
    const storage = new MemoryInventoryStorage();
    const kv = new RecordingKv();
    const resume = { async resumeAlarms() {} };

    await prepareAppDeletionSaga(storage, kv, "app-A", "generation-A", 5_000, resume);
    await prepareAppDeletionSaga(storage, kv, "app-A", "generation-B", 6_000, resume);

    await expect(
      beginOrResumeAppDeletionCancelSaga(storage, kv, "app-A", resume, "generation-A"),
    ...
    await expect(markAppDeletionSagaD1Deleted(storage, "app-A", "generation-A")).rejects.toThrow(
      /generation does not match/u,
    );
    await expect(
      advanceAppDeletionFinalizeSaga(storage, "app-A", "generation-A", {
        async purgeEntity() {},
      }),
    ).rejects.toThrow(/generation does not match/u);

    await markAppDeletionSagaD1Deleted(storage, "app-A", "generation-B");
    await expect(
      advanceAppDeletionFinalizeSaga(storage, "app-A", "generation-B", {
        async purgeEntity() {},
      }),
...
describe("holdover write App deletion saga recovery", () => {
  it("prepare leaves DO suppressed when KV put fails, then durable cancel restores", async () => {
    ...
    await expect(
      prepareAppDeletionSaga(storage, kv, "app-A", GENERATION_ID, 5_000, null),
    ).rejects.toThrow(/KV freeze failed and cancel is incomplete|forced KV put failure/);
  ...
  it("cancel checkpoints mid-list resume and KV delete failures then recovers without a request", async () => {
    ...
    await registerAppInventoryEntity(storage, { idType: "user", targetingKeyHash: "hash-2" });
    const kv = new RecordingKv();
    await prepareAppDeletionSaga(storage, kv, "app-A", GENERATION_ID, 9_000, {
      async resumeAlarms() {
        return undefined;
      },
    });
    ...
    await expect(
      beginOrResumeAppDeletionCancelSaga(storage, kv, "app-A", resume, GENERATION_ID),
    ).rejects.toThrow(/forced KV delete failure/);
  ...
  it("finalize after D1 boundary refuses cancel and resumes after Entity purge failure", async () => {
    ...
    await registerAppInventoryEntity(storage, { idType: "user", targetingKeyHash: "hash-1" });
    const kv = new RecordingKv();
    await prepareAppDeletionSaga(storage, kv, "app-A", GENERATION_ID, 7_000, {
      async resumeAlarms() {
        return undefined;
      },
    });
    await markAppDeletionSagaD1Deleted(storage, "app-A", GENERATION_ID);
    ...
    await expect(
      advanceAppDeletionFinalizeSaga(storage, "app-A", GENERATION_ID, purge),
    ).rejects.toThrow(/forced entity purge failure/);
