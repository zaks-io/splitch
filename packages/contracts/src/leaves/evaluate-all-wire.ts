...
export const EvaluateAllEntrySchema = EvaluateAllEntryBaseSchema.refine(
  ...
  .refine(
    (entry) =>
      entry.reason === "SPLIT" || entry.reason === "DEFAULT" || entry.exposureTicket === null,
    { message: "exposureTicket is only allowed when reason is 'SPLIT' or 'DEFAULT'" },
  )
  .refine(
    (entry) =>
      entry.reason === "SPLIT" || entry.reason === "DEFAULT" || entry.exposureIdentity === null,
    { message: "exposureIdentity is only allowed when reason is 'SPLIT' or 'DEFAULT'" },
  )
