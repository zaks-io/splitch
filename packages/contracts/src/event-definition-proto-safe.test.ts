...
describe("Closed JSON own-property required declarations", () => {
  it.each(PROTOTYPE_NAMES)(
    "rejects required %s unless properties has an own matching schema",
    (name) => {
  ...
  it.each(["constructor", "toString"] as const)(
    "accepts required %s when properties has an own matching schema",
    (name) => {
  ...
  it.each(PROTOTYPE_NAMES)(
    "rejects nested required %s unless the nested properties has an own matching schema",
    (name) => {
      const parsed = ClosedJsonSchemaSchema.safeParse(
