...
import type { ExperimentConfig, FlagConfig, Provider } from "./types";

export function parseConfigSnapshot(payload: string, source: string): ConfigSnapshot {
  let value: unknown;
  try {
    value = JSON.parse(payload);
  } catch (cause) {
    throw new ProviderError(`Stored ${source} configuration is malformed JSON`, { cause });
  }
  const parsed = ConfigSnapshotSchema.safeParse(value);
  if (!parsed.success)
    throw new ProviderError(`Stored ${source} configuration is invalid: ${parsed.error.message}`);
  assertSnapshotReferences(parsed.data, source);
  return parsed.data;
}

export function configSnapshotProvider(snapshot: ConfigSnapshot, source: string): Provider {
  ...
  return {
    async getFlag(appId, environmentId, flagKey) {
      assertScope(snapshot, appId, environmentId, source);
      const flag = flags.get(flagKey);
      if (!flag)
        throw new ProviderError(`Flag "${flagKey}" is absent from the ${source} snapshot`, {
    ...
    async getExperiment(appId, environmentId, experimentId) {
      ...
      if (!experiment)
        throw new ProviderError(
          `Experiment "${experimentId}" is absent from the ${source} snapshot`,
        );
      return experiment;
    },
  };
...
function resolveFlag(snapshot: ConfigSnapshot, flag: FlagConfigKV): FlagConfig {
...
function assertScope(
  ...
  if (snapshot.appId !== appId || snapshot.environmentId !== environmentId)
    throw new ProviderError(`${source} snapshot scope does not match the installed integration`);
}

function assertSnapshotReferences(snapshot: ConfigSnapshot, source: string): void {
  uniqueScopedIds(
  ...
  const runIds = uniqueRunIds(snapshot, experimentIds, source);
...
function uniqueScopedIds(
  ...
  for (const value of values) {
    ...
    if (ids.has(value.id))
      throw new ProviderError(`Duplicate ${kind} "${value.id}" in ${source} snapshot`);
    ids.add(value.id);
  }
  return ids;
}
...
function uniqueRunIds(
  ...
  for (const run of snapshot.runs) {
    if (ids.has(run.id)) throw new ProviderError(`Duplicate Run "${run.id}" in ${source} snapshot`);
