...
import { ALLOWED_PUBLISHABLE_PACKAGE } from "./policy/constants.mjs";
...
import { lintSdkReleaseMetadata } from "./policy/sdk-metadata.mjs";
...
describe("lintWorkspacePublishability", () => {
  it("passes when only @splitch/sdk is publishable", () => {
  ...
  it("fails when a non-sdk workspace is publishable", () => {
    ...
    expect(violations[0]?.message).toContain("only @splitch/sdk may be published");
  });
...
describe("lintSdkReleaseMetadata", () => {
  it("accepts the valid SDK fixture", () => {
    expect(lintSdkReleaseMetadata(pkg("packages/sdk/package.json", validSdkManifest))).toEqual([]);
  });

  it("fails when SDK depends on a private workspace package", () => {
    const violations = lintSdkReleaseMetadata(
...
describe("repo publishing policy against the live monorepo", () => {
  ...
  it("only allows @splitch/sdk to be publishable", async () => {
    const violations = await lintPublishingPolicyFromRepo(repoRoot);
    ...
    expect(ALLOWED_PUBLISHABLE_PACKAGE).toBe("@splitch/sdk");
  });
});
