...
describe("Cloudflare integration repository", () => {
  ...
  it("scopes the list on the App and on the Environment independently", async () => {
    ...
    await install("a", FIRST_INSTALLATION_ID);
    await repo.cloudflare.createInstallation(envScope(seed.a.appId, SIBLING_ENVIRONMENT_ID), {
    ...
    const own = await repo.cloudflare.listInstallations(
    ...
    const mismatched = await repo.cloudflare.listInstallations(
