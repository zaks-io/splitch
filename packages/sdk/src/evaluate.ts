...
import { resolveIdempotencyKey } from "./evaluate-idempotency-key";
...
export interface EvaluationContext {
  ...
  /**
   * Optional retry identity. The SDK generates a key per call when omitted.
   * Pass the same key across application retries of one logical Evaluation.
   */
...
function replayCachedHit(
  deps: EvaluateDeps,
  flagKey: string,
  context: EvaluationContext & { readonly idempotencyKey: string },
 ...
 *
 *   1. Default idType to 'user'; resolve the Default Variant and generate or
 *      validate the call's idempotencyKey.
...
export async function runEvaluate(
  ...
  const resolvedContext = {
    ...context,
    idempotencyKey: resolveIdempotencyKey(deps, flagKey, context),
  };

  const lookup = deps.seenSet.get(flagKey, idType, targetingKey, attributes, deps.now());
  if (lookup.kind === "hit") {
    return replayCachedHit(deps, flagKey, resolvedContext, lookup.entry, defaultValue);
  }
  ...
  const result = await deps.transport.evaluate({
    ...requestFor(flagKey, resolvedContext),
  });
