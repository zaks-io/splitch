...
import { createPanelCloudflareClient } from "./panel-cloudflare";
...
const INSTALLATION = {
  ...
  endpoint: "https://flags.example.workers.dev",
...
describe("panel Cloudflare client", () => {
  ...
  it("reads a 204 revoke as the success it is", async () => {
    ...
    expect(String(fetch.mock.calls[0]?.[0])).toBe(
      `https://control-plane.internal/apps/app_1/envs/env_prod/integrations/cloudflare/installations/${INSTALLATION_ID}`,
    );
