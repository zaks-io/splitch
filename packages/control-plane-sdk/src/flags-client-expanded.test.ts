...
describe("control plane sdk Variant catalog operations", () => {
  ...
  it("patches a Variant by name and returns the whole Flag", async () => {
    const { sdk, requests } = sdkWith(() => Response.json(flag));
    ...
    await expect(requests[0]?.json()).resolves.toEqual({ description: "the new checkout" });
...
describe("control plane sdk targeting and promotion", () => {
  it("replaces the targeting rules for one Environment's Flag Configuration", async () => {
    const { sdk, requests } = sdkWith(() => Response.json(config));
    ...
    await expect(requests[0]?.json()).resolves.toEqual({ targetingRules: [] });
  ...
  it("surfaces the Env Policy's confirm requirement rather than silently promoting", async () => {
    const confirmationRequired = {
      code: "CONFIRMATION_REQUIRED",
      message: "Env Policy requires confirm for enabledState",
      details: {
        gate: "enabled_state",
        environmentId: "env_prod",
        attemptedOp: "flags_promote",
        recommendedAction: "RETRY_WITH_CONFIRMATION",
      },
    };
    const { sdk } = sdkWith(() => Response.json(confirmationRequired, { status: 428 }));
    ...
    ).resolves.toEqual({ ok: false, status: 428, error: confirmationRequired });
  });
});
