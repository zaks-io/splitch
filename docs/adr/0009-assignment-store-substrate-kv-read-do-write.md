...
**Fine-grained DOs are the documented pattern, not an anti-pattern.** Cloudflare supports an _unlimited_
number of Durable Objects ("millions of objects... scale horizontally"); idle DOs are evicted and incur no
compute/duration cost (only bytes at rest). The _only_ warned-against grain is the opposite — a single hot
...
per-key DO's inability to _enumerate_ an Entity's assignments doesn't bite us: enumeration is the `getAll`
...
- **DO read-fallback on KV miss** (read KV, hop to the DO on a miss to close the boundary window) —
  rejected: a KV miss is the _normal_ case for every genuinely new Entity, so this pays a DO hop on the
...
**cosmetic and self-healing**, and bounded to a narrow intersection — _returning Entity × live Run boundary ×
cross-POP × within the propagation window_. For a brand-new Entity it cannot occur: `assign()` is
