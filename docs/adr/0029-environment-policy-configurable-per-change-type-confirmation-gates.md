...
imposed. Different users will want to gate different things: some want a Confirmation on *any* prod change,
...
a flag *off* regardless of Policy — incident control is never gated.
...
unusual flows (a `canary` env stricter than `prod`, a `dev` that *is* gated for a regulated team). A
...
- **A single per-Environment boolean "require confirmation"** — rejected as too coarse. The user wants to
  confirm *some* change types and not others in the same Environment; one boolean can't express "confirm
  availability changes but not value tweaks."
- **Full approval workflow (second-principal sign-off) in v1** — deferred, not rejected. `approve` is
  reserved as a Policy *level* above `confirm`, so adding multi-party approval later is an additive Policy
  value, not a new mechanism. v1 ships `allow` and `confirm`.
...
- **Each Environment carries an Environment Policy** — a map from change type to `allow | confirm`
  (`| approve`, future). Stored per Environment, enforced server-side at the commit seam for *all* surfaces
  (panel, CLI, MCP, agent).
