...
holdover's Exposures attributed to the *old* Run rather than accruing into the new one — is splitch's own
...
`(Experiment, Targeting Key) -> (runId, Variant)`, written at first Exposure. The *storage* is the same kind
