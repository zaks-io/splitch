...
prior Variant — sticky *experience*, to avoid a jarring mid-experiment flip — but its Exposures stay
attached to the Run it was first exposed under and are **not re-counted** in the new live Run. New (and
never-exposed) Entities flow into the new Run cleanly. This separates *assignment-for-experience* from
*assignment-for-analysis*, preserving both Run-dataset purity (ADR-0002) and a non-jarring UX, which the
...
`(Experiment, Targeting Key) -> (runId, Variant)`, written at first Exposure (the equivalent of Statsig
Persistent Assignment / GrowthBook Sticky Bucketing). It needs a low-latency, edge-local read on the
evaluate path — a hard constraint on the Provider/storage seam, to be designed there.
