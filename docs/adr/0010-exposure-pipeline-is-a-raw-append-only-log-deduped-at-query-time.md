...
*input* to an ELT deduper, not a problem to fix upstream.
...
- **Streaming dedup at ingest (ETL, collapse early)** — rejected: lossy and not replayable (a bad collapse
  ...
  best-effort *cost* optimization over a raw store that remains the source of truth — never as the
  authority.
...
*purely* to cut volume if it ever hurts — explicitly non-authoritative — but it is speculative complexity
until volume demands it, so it is not in v1. Every analysis query reads through the shared dedup
