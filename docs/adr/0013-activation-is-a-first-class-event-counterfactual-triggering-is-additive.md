...
**Production target, stated plainly:** v1 is a production-ready foundation on the *final* schema, not a
half-built prototype to be thrown away. The guarantee this ADR makes is **no rewrite** — the hardest
...
(ADR-0010), carrying `(Entity, Run, activation_ts)` — *not* a flag derived inside a query. This is the load-
...
Treatment Entities that activated AND Control Entities that *would have* activated) is implemented as the
Control arm emitting an activation-shaped event with a `counterfactual: true` marker. That is a new column
*value*, flowing through the **same** log, the **same** join, the **same** anchor, and the **same** SRM —
zero schema change, zero query rewrite.

## What is and isn't in v1

- **In v1 (production, final schema):** activation as a first-class event; the gate query; the
  `activation_ts` re-anchor; both bias guardrails (ADR-0012). Correct for variant-independent activations,
  loud for the rest.
- **Deferred (additive later):** the SDK-side Control-arm *would-have-activated evaluation logic* — the one
...
- **Build the SDK counterfactual evaluation in v1 too** — rejected: no SDK/pipeline/experiment exists to
  build or test it against, so it is speculative, untestable machinery. The schema (this ADR) is what makes
  v1 production-ready and rewrite-proof; the blind logic is not.
...
The activated-population SRM (ADR-0012) is the signal that tells you *when* a real experiment needs
counterfactual triggering: a gated-scorecard SRM with no full-exposed SRM means the gate is Treatment-
affected, and that experiment is the one that justifies building the deferred logic. Until then, v1 gates
