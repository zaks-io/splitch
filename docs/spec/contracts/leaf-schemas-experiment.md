...
| `targetingKeyType`   | `string`            | yes      | **Entity type** the key identifies; stamped as `id_type` on every Exposure and validated against requests; changing it requires a new Run. Create/patch constrain the value to `'user' \| 'session' \| 'workspace'` (see note below) |
...
Create/patch write boundary for `targetingKeyType`: `'user' | 'session' | 'workspace'`.
The Experiment/Run leaf and storage shapes keep a plain `string` so historical rows remain
readable; an unrecognized value is rejected at create/patch with the accepted set named in the
validation issue.
