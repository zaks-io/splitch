...
| `targetingKeyType`   | `TargetingKeyType`  | yes      | **Entity type** the key identifies; stamped as `id_type` on every Exposure and validated against requests; changing it requires a new Run. Write-time enum: `'user' \| 'session' \| 'workspace'` |
...
`TargetingKeyType` enum (Experiment create/patch write boundary): `'user' | 'session' | 'workspace'`.
Leaf and storage shapes keep a plain string so historical rows remain readable; an unrecognized
value is rejected at create/patch with the accepted set named in the validation issue.
