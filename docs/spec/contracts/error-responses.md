...
RecommendedAction =
  ...
  | 'READ_PER_ENVIRONMENT'   // the App-wide read is over a fan-out budget; read the same signal one Environment at a time (no retry helps)
```
...
| `ATTENTION_FANOUT_LIMIT_EXCEEDED` | `READ_PER_ENVIRONMENT`    | the App-wide attention rollup exceeds a fan-out budget; list Experiments per Environment instead. Retrying the rollup never clears it |
