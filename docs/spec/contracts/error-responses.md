...
ErrorCode =
  ...
  | 'UNSUPPORTED_MEDIA_TYPE'      // request body is not application/json
  ...
  | 'EXPOSURE_TICKET_INVALID'     // Exposure Ticket failed MAC/tenant/shape verification (sdk/exposures-endpoint.md)
  | 'EXPOSURE_TICKET_EXPIRED'     // Exposure Ticket older than its TTL; refetch Precomputed Evaluations
  | 'SERVER_EXPOSURE_INVALID'     // trusted adapter Exposure failed immutable Run verification
  | 'UNSUPPORTED_OBJECT_KEY'      // a JSON own "__proto__" key that zod would silently drop (evaluations / attributes)
  ...
  | 'MEMBERSHIP_CONFLICT'        // User is already a member; add never changes their role
  | 'EXPERIMENT_KEY_CONFLICT'    // Experiment key still held (live or archived) in this Environment
  ...
  | 'LAST_OWNER_REQUIRED'         // would leave a shared Org, or an App, without an owner
  ...
  | 'DECISION_BLOCKED'            // one or more decision-valid health or power checks failed
  | 'DECISION_RESULT_UNAVAILABLE' // selected Run has no decision-bearing result envelope
  ...
  | 'QUOTA_EXCEEDED'                  // User owns the maximum number of non-provisional Organizations
  | 'ACTIVATION_NOT_AVAILABLE'    // Metric Event cannot activate a Run; coarse and non-retryable
...
| `EVENT_SCHEMA_MISMATCH`            | `{ eventName: string, eventDefinitionVersionId?: string, issues: Array<{ path: string[], message: string }> }` — paths identify unknown, missing, type-invalid, allowlist-invalid, or range-invalid fields, Dimensions, or nested JSON keys. Client Key responses keep only caller-owned issue paths and generic non-revealing copy. They omit `eventDefinitionVersionId`, configured field names the caller did not send, expected types, bounds, allowed values, and Entity types. A trusted server credential may include those diagnostics. The Worker retains the full untruncated configured definition, IDs, and original issues in a trusted server-side diagnostic sink.                                                                                                                                                                                                                                                                                    |
| `ENTITY_TYPE_MISMATCH`             | `{ expectedIdType?: string \| null, receivedIdType: string, eventDefinitionId?: string, metricId?: string, runId?: string }` — null `expectedIdType` means the accepting `web` definition prohibits Entity identity. Client Key responses keep only caller-owned `receivedIdType`. A trusted server credential may include `expectedIdType` and `eventDefinitionId`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `RESOURCE_NOT_EMPTY`               | `{ resourceType: 'app' \| 'environment' \| 'flag' \| 'variant' \| 'organization' \| 'segment', resourceId: string, childType: string, childCount: number, childCounts?: Record<string, number>, attemptedOp: string, blockers?: Array<{ resourceType: 'app' \| 'environment' \| 'organization', resourceId: string, childType: string, children: Array<{ id: string, removeCommand: string }> }>, targetingRuleIds?: string[], targetingRules?: Array<{ id: string, environmentId: string }> }` — `childCount` counts `childType`; `childCounts` reports each unlike blocker type when needed. `blockers` carries the full tree with child IDs and CLI remove commands (CLI vocabulary) for App/Environment deletes; flag/variant emptiness guards may omit it. Variant delete blocked by `targeting_rules.variant_id` sets `childType` to `flag-targeting-rules` and names those rules in `targetingRuleIds` / `targetingRules` (IDs are scoped to an Environment). |
| `MEMBERSHIP_CONFLICT`              | `{ existingRole: 'owner' \| 'admin' \| 'member' }`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `EXPERIMENT_KEY_CONFLICT`          | `{ key: string, status: 'draft' \| 'running' \| 'ended' \| 'archived', archivedExperimentId?: string, recommendedAction: 'CHOOSE_DIFFERENT_KEY' }`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `SELECTOR_AMBIGUOUS`               | `{ candidates: Array<{ orgSlug: string, appId: string, appSlug: string } \| { environmentId: string, environmentKey: string }>, recommendedAction: 'USE_CANONICAL_ID' }`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `EVENT_DEFINITION_UNPUBLISHED`     | `{ eventDefinitionId?: string, eventName: string }` — Client Key responses keep caller-owned `eventName` and omit `eventDefinitionId`. A trusted server credential may include the ID.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `EXPOSURE_TICKET_INVALID`          | `{ exposureId: string }`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `EXPOSURE_TICKET_EXPIRED`          | `{ exposureId: string, issuedAt: string }`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `SERVER_EXPOSURE_INVALID`          | `{ exposureId: string, reason: 'SCOPE_MISMATCH' \| 'RUN_NOT_LIVE' \| 'CONFIG_HASH_MISMATCH' \| 'VARIANT_MISMATCH' \| 'TIMESTAMP_OUT_OF_BOUNDS' }`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `UNSUPPORTED_OBJECT_KEY`           | `{ key: string, path: string[] }` — a JSON own `"__proto__"` key that the Worker Zod record parser would otherwise silently drop (Precomputed Evaluations Flag Keys or Evaluation Context attributes)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `UNSUPPORTED_MEDIA_TYPE`           | `{ receivedMediaType: string \| null, supportedMediaTypes: string[] }` — a body-bearing registrar mutation did not declare a supported JSON media type                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `INSUFFICIENT_SCOPES`              | `{ requiredScopes: string[], heldScopes: string[] }`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `LAST_OWNER_REQUIRED`              | `{ orgId: string }` or `{ appId: string }` (App membership)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `IDEMPOTENCY_KEY_CONFLICT`         | `{ scope: 'approval_request' \| 'review' \| 'conclusion' \| 'app_create' \| 'flag_create' \| 'convex_installation' \| 'convex_secret_rotation' \| 'convex_evaluation', idempotencyKey: string }`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `DECISION_BLOCKED`                 | `{ runId: string, resultToken: string, dataWatermark: string, failures: DecisionFailure[] }`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `DECISION_RESULT_STALE`            | `{ runId: string, expectedResultToken: string, currentResultToken: string }`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `DECISION_RESULT_UNAVAILABLE`      | `{ runId: string, envelopeState: 'ready' \| 'no_data' \| 'no_run' }` — `ready` means the ordinary Results read did not carry the all-or-nothing conclusion evidence pair                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `TARGET_CONFIGURATION_STALE`       | `{ flagId: string, environmentId: string, expectedConfigVersion: number, currentConfigVersion: number, recommendedAction: 'REFRESH_AND_REPROPOSE' }`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `QUOTA_EXCEEDED`                   | `{ resourceType: "organization", currentCount: number, ceiling: number, recommendedAction: "REDUCE_OWNED_ORGANIZATIONS" }` — the User owns the complete session-safe number of non-provisional Organizations; member and admin memberships do not count                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `SERVICE_UNAVAILABLE`              | `{ retryAfterMs: number, mutationCommitted?: true }`. Provider unresolvable; mirrors the `Retry-After` response header. `mutationCommitted` means the requested mutation crossed its durable boundary before cleanup became unavailable.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `ACTIVATION_NOT_AVAILABLE`         | `{}`. The Metric Event cannot activate a matching Experiment Run. Client Key responses deliberately collapse unpublished Activation configuration, no matching binding, incompatible Entity type, and no Exposure-backed Assignment into this one shape. API Key responses may name the permanent failed condition in `message`, and receive `SERVICE_UNAVAILABLE` instead when the only missing piece is an Exposure-backed Assignment, which races an asynchronous Exposure drain (ADR-0058). The 409 `ACTIVATION_NOT_AVAILABLE` response itself advertises no retry delay.                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `CREDENTIAL_REVOKED`               | `{}`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `FORBIDDEN`                        | `{}`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `INTERNAL_SERVER_ERROR`            | `{ fault?: string }` — optional named seam; `{}` still valid when only the message carries the reason                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |

---

## Decision failure codes

`DECISION_BLOCKED.details.failures` is a discriminated union on `code`. It reports every applicable
failure from the same decision-gate evaluation, so fixing one check cannot silently reveal another on
retry.
...
type DecisionFailure =
  | {
      ...
      details: {
        ...
        frozenVariantNames: string[];
        reason: "absent_from_frozen_variant_set" | "unreadable_frozen_variant_set";
      };
    }
  | {
      code: "DECISION_RESULT_INVALID";
      checkIds:
        ["engine_status"] | ["decision_valid_result"] | ["engine_status", "decision_valid_result"];
      details: { members: Array<{ metricId: string; variant: string; status: StatsResultStatus }> };
    }
  | {
      code: "DECISION_UNDERPOWERED";
      checkIds: ["underpowered"];
      details: {
        members: Array<{ metricId: string; variant: string; status: StatsResultStatus }>;
        lowN: boolean;
      };
    }
  | {
  ...
  | {
      ...
      details: { pValue: number | null; rates: Record<string, number> | null };
    };
```
...
add its mapping first.

These uppercase snake-case failure codes are closed and machine-stable. They are not warning labels.
Guardrails and `__multiple__` are not in this failure union because the shipped decision gate does not
block on them. The exact gate conditions and no-override invariant live in
[../control-plane/conclusion-and-winner-promotion.md](../control-plane/conclusion-and-winner-promotion.md#result-identity-and-decision-gate).
...
RecommendedAction =
  ...
  | 'CHOOSE_DIFFERENT_SLUG'  // organization slug is taken; resend create with a different slug
...
| `EXPERIMENT_KEY_CONFLICT`         | `CHOOSE_DIFFERENT_KEY`       | a live or archived Experiment still holds `(app, env, key)`; resend create with a different key (keys are not freed on archive)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `SELECTOR_AMBIGUOUS`              | `USE_CANONICAL_ID`           | choose the intended App or Environment candidate and retry with its canonical ID; pass `by=id` when the Environment ID is itself the ambiguous selector                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `QUOTA_EXCEEDED`                  | `REDUCE_OWNED_ORGANIZATIONS` | transfer or delete an owned Organization before creating another                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `TARGET_CONFIGURATION_STALE`      | `REFRESH_AND_REPROPOSE`      | read the current target Flag Configuration and submit the intended complete proposal against its current version                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
- `EVENT_SCHEMA_MISMATCH` — unknown, missing, type-invalid, allowlist-invalid, or range-invalid
  field, Dimension, or nested JSON key. Client Key details keep `eventName` and caller-owned issue
  paths with generic copy; they omit Event Definition Version IDs, configured field names the
  caller did not send, expected types, configured bounds, allowed values, and Entity types.
- `ENTITY_TYPE_MISMATCH` — request `idType` does not equal the accepting version's `entityType`.
  Client Key details keep only `receivedIdType`.
...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------- |
| `VALIDATION_ERROR`, `ALLOCATION_INVALID`, `INVALID_*`, `EVENT_SCHEMA_MISMATCH`, `ENTITY_TYPE_MISMATCH`, `EXPOSURE_TICKET_INVALID`, `SERVER_EXPOSURE_INVALID`, `UNSUPPORTED_OBJECT_KEY`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | 400         |
| `UNAUTHORIZED`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | 401         |
| `CREDENTIAL_REVOKED`, `FORBIDDEN`, `INSUFFICIENT_SCOPES`, `ORIGIN_NOT_ALLOWED`, `APP_MISMATCH`, `APPROVAL_REVIEW_FORBIDDEN`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | 403         |
| `*_NOT_FOUND`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | 404         |
| `RUN_FROZEN`, `DECISION_LOCKED`, `DECISION_BLOCKED`, `DECISION_RESULT_STALE`, `DECISION_RESULT_UNAVAILABLE`, `TARGET_CONFIGURATION_STALE`, `TARGETING_KEY_MISMATCH`, `RUN_NOT_RUNNING`, `EXPERIMENT_RUNNING`, `EXPERIMENT_NO_DRAFT`, `VARIANT_NOT_AVAILABLE`, `RESOURCE_NOT_EMPTY`, `MEMBERSHIP_CONFLICT`, `EXPERIMENT_KEY_CONFLICT`, `MULTIPLE_VARIANT_CONFLICT`, `ATTENTION_FANOUT_LIMIT_EXCEEDED`, `QUOTA_EXCEEDED`, `ACTIVATION_NOT_AVAILABLE`, `LAST_OWNER_REQUIRED`, `LAST_ENVIRONMENT_REQUIRED`, `PRIVACY_CONFIRMATION_REQUIRED`, `APPROVAL_REVIEW_REQUIRED`, `APPROVAL_REQUEST_STALE`, `APPROVAL_REQUEST_RESOLVED`, `APPROVAL_APPLICATION_FAILED`, `IDEMPOTENCY_KEY_CONFLICT`, `EVENT_DEFINITION_UNPUBLISHED`, `EVENT_DEFINITION_IMMUTABLE`, `EVENT_ID_CONFLICT` | 409         |
| `WEB_ANALYTICS_WINDOW_UNAVAILABLE`, `EXPOSURE_TICKET_EXPIRED`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | 410         |
