...
RecommendedAction =
  ...
  | 'READ_PER_ENVIRONMENT'   // the App-wide read is over a fan-out budget; enumerate Experiments per Environment (experiments_list), then read each running Experiment's results (experiment_results_get) for the SRM/Guardrail signal (no retry helps)
```
