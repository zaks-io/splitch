...
ErrorCode =
  ...
  | 'DECISION_RESULT_STALE'       // selected Run result changed since the caller read it
...
| `DECISION_BLOCKED`                 | `{ runId: string, resultToken: string, failures: DecisionFailure[], recommendedAction: 'READ_DECISION_DIAGNOSTICS' }`                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `DECISION_RESULT_STALE`            | `{ runId: string, expectedResultToken: string, currentResultToken: string, recommendedAction: 'REFRESH_RESULTS' }`                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
failure from the same server result, so fixing one check cannot silently reveal another on retry.

```ts
type DecisionFailure =
  | {
      code: "DECISION_RESULT_INVALID";
      details: { members: Array<{ metricId: string; variant: string; status: string }> };
    }
  | {
      code: "DECISION_UNDERPOWERED";
      details: {
        members: Array<{ metricId: string; variant: string; status: string }>;
        lowN: boolean;
      };
    }
  | {
      code: "DECISION_SRM_MISMATCH";
      details: { checks: Array<"exposure_srm" | "activated_srm"> };
    }
  | {
      code: "DECISION_ACTIVATION_IMBALANCE";
      details: { pValue: number; rates: Record<string, number> };
    }
  | {
      code: "MULTIPLE_VARIANT_CONFLICT";
      details: { multipleRate: number; threshold: 0.01 };
    }
  | {
      code: "DECISION_GUARDRAIL_BREACHED";
      details: {
        breaches: Array<{ metricId: string; variant: string; threshold: number; ciLower: number }>;
      };
    };
```

These uppercase snake-case tokens are closed and machine-stable. They are not warning labels. The
exact gate conditions and no-override invariant live in
...
RecommendedAction =
  ...
  | 'READ_DECISION_DIAGNOSTICS' // read the bound result, SRM trend, and Dimension auto-cuts; never bypass the gate
  | 'REFRESH_RESULTS'        // re-read the selected Run's server-owned result before deciding
```
...
| `DECISION_BLOCKED`                | `READ_DECISION_DIAGNOSTICS` | read the bound result and its diagnostics; no action token authorizes an override                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `DECISION_RESULT_STALE`           | `REFRESH_RESULTS`           | re-read Results for the selected Run and make the decision from the new result token                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
