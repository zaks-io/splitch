...
The `$` name prefix is reserved for the Splitch-provisioned built-in `web` definitions
(`$page_view`, `$web_vital`, `$browser_error`). User-supplied create requests reject names starting
with `$`, and the control plane rejects every edit, republish, or delete of a `$`-prefixed
definition. The prefix is the sole built-in marker; there is no ownership column.
...
The templates are the content of the built-in `$page_view`, `$web_vital`, and `$browser_error`
Event Definitions, which the control plane provisions per App with a null `entityType` and
Splitch-owned display metadata. Applications that want their own event names, display metadata, or
Entity identity expand a template into the ordinary strict Event Definition Version request under a
non-`$` name. The API never accepts a template selector.
...
| `pathname`         | `string`                                                   | yes      | Document URL path; `/`-prefixed, no query/fragment, at most 512 characters            |
| `referrerHostname` | `string`                                                   | no       | Lowercase referrer hostname, at most 253 characters; absent when empty or same-origin |
...
and retains it across retries. It also stamps `captureSource`, `sdkVersion`, `pathname`, and
`referrerHostname`; the public manual event input cannot override them. Direct HTTP callers can
report any supported capture source, bounded SDK version, and page context, so those values are
advisory rather than authenticated provenance. `pathname` must start with `/` and contain no `?`,
`#`, whitespace, or control characters; `referrerHostname` must be a lowercase hostname. Both
participate in the retry fingerprint as ordinary caller-supplied content.
...
| `visitorHash`              | `string`                                      | yes      | Daily-rotating visitor pseudonym derived at accept time  |
| `pathname`                 | `string`                                      | yes      | Validated document URL path                              |
| `referrerHostname`         | `string`                                      | no       | Validated lowercase referrer hostname                    |
| `country`                  | `string`                                      | no       | Uppercase ISO 3166-1 alpha-2 from request metadata       |
| `deviceClass`              | `string`                                      | yes      | `desktop`, `mobile`, `tablet`, or `unknown`              |
