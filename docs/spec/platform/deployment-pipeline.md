...
| `e2e`                   | nightly schedule, manual dispatch                        | `e2e-main`, queued               | wired: full-stack Control Panel Playwright harness against `main`; its latest completed run gates `deploy-production`                                                                                                                  |
...
| `deploy-production`     | dispatch from successful `ci` on `main`, manual dispatch | `production-deploy`, queued      | wired: exact-SHA validation, successful CI and fresh nightly `e2e` verification, affected-phase and Worker planning from the latest successful production deployment, conditional Tinybird/D1/Worker mutation, and Linear release sync |
...
1. Verify successful `ci` evidence for the exact release SHA. Automatic runs verify the dispatching
   ...
   without entering the GitHub `production` environment or creating a deployment record. Then verify
   the latest completed `e2e` run on `main` succeeded no more than 48 hours ago. The Playwright
   harness runs nightly instead of per push, so a red or stale nightly blocks every deploy until the
   `e2e` workflow is re-dispatched green; there is no bypass input.
