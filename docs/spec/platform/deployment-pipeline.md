...
6. Sync Worker secrets, then deploy Workers through Turborepo package deploy tasks. The Turbo graph
   ...
   The Control Panel delegation protocol uses a bounded cutover: deploy the Control Plane with the
   predecessor session-handle binding entrypoint enabled with a 30-minute expiry, deploy the V2 Panel
   bound to the signed entrypoint, then immediately redeploy the Control Plane from its checked-in
   config with predecessor session redemption disabled. Credential backfill and remaining Workers run
   only after that final state is active.
