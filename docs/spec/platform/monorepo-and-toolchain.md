...
| Event Ingest Worker      | Append-only event intake     | Evaluation usage and Exposure/Activation/Metric/Web Event validation, version stamping, per-scope SQLite Admission Gate DOs, four datasource-specific durable queues, sharded DO dedup/outbox, bounded Tinybird NDJSON microbatch delivery | Variant resolution, result calculation, control-plane CRUD       |
