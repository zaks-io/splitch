...
| `rawBodyByteLimit` | Optional raw UTF-8 byte cap. Mutating JSON routes get a conservative registrar default when this is omitted.  |
...
2. Enforce a raw-body byte limit before buffering or parsing JSON. Mutating
   ...
   smaller explicit cap) and otherwise a conservative registrar default. An
   oversized `Content-Length` is rejected without reading the body; an
   oversized chunked body stops during the stream. GET routes do not buffer a
   body. No mutating registrar route silently opts into an unbounded buffer.
