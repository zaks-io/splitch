...
| `client_keys`        | Client Key records (public key material + revocation)                    |
| `api_keys`           | API Key records (hash only; secret never stored)                         |
| `flags`              | Flag definitions + Variants + Targeting Rules (as JSON columns)          |
| `experiments`        | Experiment records + draft assignment config                             |
| `runs`               | Run records (frozen assignment config snapshot, status, timestamps)      |
...
Exception: `live_run:{app_id}:{experiment_id}` → `run_id` is written by the control-plane at Publish
time (see [run-state-machine.md](run-state-machine.md)) for the edge to read. This is a one-way write;
the control-plane does not read it back.
...
```
{
  ...
  action:      string     // e.g. "flag.create", "run.publish", "api_key.revoke"
  resource_id: string     // the ID of the affected entity
  resource_type: string   // "flag" | "experiment" | "run" | ...
...
control-plane Worker which injects `app_id` from the auth token context. The Worker is the single
enforcement point for Tinybird isolation.
