...
| `run_an_experiment`  | `flagId`, `flagKey`, `variants`, `allocation` | `experiments_create` using `flagId` → `experiments_start` → `flags_test_eval` using `flagKey` → poll `experiment_results_get`                             |
| `end_a_run`          | `runId`, `flagKey`                            | `flags_test_eval` using `flagKey` → `runs_end` using `runId` → confirm `RUN_NOT_RUNNING` is now the state                                                 |
...
-------------------------    -----------------------------------------------------------
CREATE_NEW_RUN             → flags_list (retain Flag key) → experiments_create (clone) → apply the blocked change → experiments_start → flags_test_eval
