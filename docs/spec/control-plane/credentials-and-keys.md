...
| capability         | Evaluate-only, App-scoped                        | Full data-plane access, App-scoped        |
...
Value: { app_id, revoked: boolean, origin_allowlist: string[] | null, valid_until: ISO8601 }
```

**API Key cache entry:**
```
Key:   ak:{key_hash}
Value: { app_id, scopes: string[], revoked: boolean, valid_until: ISO8601 }
```
