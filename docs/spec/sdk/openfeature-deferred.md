...
The v1 SDK is a thin JS/TS HTTP client — `evaluate` and `peekVariant`.
...
- Single-flag-per-call (no batch evaluate in v1)
- In-memory seen-set (LRU, per-instance)
- Client Key / API Key auth (runtime-appropriate)

## What is explicitly deferred (out of scope for v1)

The following are NOT built in v1. Do not scaffold, stub, or add TODOs for these in
SDK code unless a separate decision record reopens them.
...
surface to be decided on its own." v1 does not conform to the OpenFeature SDK specification.

**2. Language clients beyond JS/TS**
v1 is JS/TS only (server-side Workers + browser). Go, Python, and other language clients are
future work.

**3. OpenFeature hook lifecycle**
OpenFeature defines before/after/error/finally hooks on the evaluation lifecycle. v1 has no
...
render). The endpoint contract (`POST /evaluate-batch`) is not defined in v1. This is a pure
...
first-call latency. Not in v1.

**6. Targeting Context schema validation**
OpenFeature defines a typed `EvaluationContext` with well-known fields. In v1, the
`evaluationContext` is partially constrained: `{ targetingKey: string, [key: string]: unknown }`.
Full schema validation (per-Flag or per-Experiment attribute typing) is deferred.

**7. OpenFeature `track()` method**
OpenFeature defines a `track(eventName, context, details)` method for metric events. v1
fires Exposures automatically on `evaluate`; no explicit tracking surface is defined.

## Why deferred

The thin client (v1) unblocks data-plane testing and agent-first SDK wiring without
...
single place that records the v1 deferral, so it is never re-discovered as an accidental gap.
