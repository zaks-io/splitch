...
The Analysis Worker reads from `metric_events`, not the Exposure/Activation `raw_events` log. It
joins only rows with the same App, Environment, `id_type`, and `targeting_key_hash` as the Run.
`id_type` must equal the Run's `targeting_key_type`; incompatible configuration fails loud.
